		...
		if( deferrCharts() && ! table_output) {
			...
			try {
				...
				for(Element add_data : ElementSet.select(source, new Match(CHART_LOC, "AddData"))) {
					RecordSet set2 = addFilterElementSet(set.copy(), ElementSet.select(add_data, new Match(FILTER_LOC, FILTER_ELEMENT)));
				...
				if( extra != null) {
					for(Element add_chart: ElementSet.select(source, new Match(CHART_LOC, extra))) {
						RecordSet set2 = addFilterElementSet(set.copy(), ElementSet.select(add_chart,new Match( FILTER_LOC, FILTER_ELEMENT)));
						...
						for(Element add_data : ElementSet.select(add_chart, new Match(CHART_LOC, "AddData"))) {
							RecordSet set3 = addFilterElementSet(set2.copy(), ElementSet.select(add_data, new Match(FILTER_LOC, FILTER_ELEMENT)));
