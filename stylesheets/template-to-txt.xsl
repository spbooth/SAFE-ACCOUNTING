...
<xsl:stylesheet 
	...
	<xsl:strip-space elements="rep:Report"/>
	
	<!-- Makes sure the elements which can remain do perserve their spaces -->
	<xsl:preserve-space elements="rep:Title rep:Heading rep:Section rep:SubSection rep:Text rep:th rep:td title heading section subsection p para text th td"/>
	...
	<!-- tables map onto  tables as one might expect -->
